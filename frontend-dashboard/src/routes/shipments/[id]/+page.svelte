    ...
    <div class="details-container">
        ...
        <div class="details-section">
            ...
            <p>
                ...
                {shipmentDetails.departure_address.locality_name}, {shipmentDetails
                    .departure_address.street}
                {shipmentDetails.departure_address.number}
            </p>
            <p>
                <strong>Destino:</strong>
                {shipmentDetails.delivery_address.locality_name}, {shipmentDetails
                    .delivery_address.street}
                {shipmentDetails.delivery_address.number}
            </p>
            <p>
                <strong>Estado Actual:</strong>
                {statusNames[shipmentDetails.status] || shipmentDetails.status}
            </p>
            <p>
                <strong>Tipo de Transporte:</strong>
                {transportTypeNames[shipmentDetails.transport_type] ||
                    shipmentDetails.transport_type}
            </p>
            <p>
                <strong>Costo Total:</strong>
                {shipmentDetails.total_cost}
            ...
            <p>
                ...
                {formatDate(shipmentDetails.created_at)}
            </p>
            <p>
                <strong>Última Actualización:</strong>
                {formatDate(shipmentDetails.updated_at)}
            </p>
            <p>
                <strong>Fecha Estimada de Entrega:</strong>
                {formatDate(shipmentDetails.estimated_delivery_at)}
            </p>
        </div>
                ...
                <ul>
                    {#each shipmentDetails.logs as log}
                        <li>
                            ...
                            {statusNames[log.status] || log.status} - {log.message}
                        </li>
                    {/each}
                </ul>
            {:else}
        ...
        <a href="/dashboard/shipments" class="button">Volver al Listado</a>
    </div>
{:else}
    <p>Cargando...</p>
{/if}
